...
export default function Portfolio() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      return await portfolio.get(
        `${process.env.RESTURL_PORTFOLIO}/portfolio?start=${currentPage}&limit=6`,
      );
    };
    fetchData()
      .then((res) => {
  ...
  return (
    <Layout>
      ...
      />
      <div className="grid grid-cols-1 gap-y-20 lg:grid-cols-3 lg:gap-y-0 lg:gap-x-8 relative -mt-32 max-w-8xl mx-auto pt-8 md:pt-26 relative z-10 md:pb-12 pb-32 px-4 sm:px-6 lg:px-8 lg:w-9/12 mt-4">
        {renderPortFolio()}
        {hasError && <ErrorComp />}
        {moreRows && (
          <>
